...
Назначение: ТОЛЬКО решение рабочих вопросов по регламентам РЕГ-001..010
с автогенерацией служебных записок, актов и заявлений.
"""
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "Я помогу решить любой рабочий вопрос на основе действующих стандартов и регламентов компании (`РЕГ-001`..`РЕГ-010`).\n\n"
        "📌 **Что я делаю:**\n"
        "1. Даю четкий пошаговый порядок действий (что делать сейчас и какие действия запрещены).\n"
        "2. Указываю точные пункты регламентов компании.\n"
        "3. **Автоматически формирую готовый документ:** служебную записку, коммерческий акт дефектовки, заявление на отпуск/командировку или заявку на ГСМ.\n\n"
...
async def process_consultant_request(user_text: str, author_name: str) -> str:
    ...
    prompt = (
        ...
        f"1. Дай пошаговый порядок действий: что разрешено делать прямо сейчас, а что категорически запрещено.\n"
        f"2. Укажи нормативное обоснование со ссылками на пункты регламентов РЕГ-001..010.\n"
        f"3. Если ситуация требует оформления документа — выдай полностью заполненный текст служебной записки, коммерческого акта или заявления с авто-подстановкой данных сотрудника.\n"
        f"Отвечай строго на русском языке, четко, понятно и без воды."
    )
...
def main():
    if not TOKEN:
        print("Ошибка: Не указан токен для второго бота!")
        return
    logger.info("Запуск бота-консультанта Star Building...")
